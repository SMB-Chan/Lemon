...
- `direct-save-v1` 対応時、File System Access API の保存先へ受信チャンクを順次書き込み
...
`showSaveFilePicker()` はすべてのブラウザで使えるAPIではありません。直接保存は、安全なコンテキストでAPIが利用可能かつ相手も必要なcapabilityを広告した場合だけ表示されます。
...
- safe/secure context
- 認証済み相手が `flow-control-v1` と `direct-save-v1` を広告
