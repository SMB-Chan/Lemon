...
assert.match(runner, /authenticated connection/i, 'retry boundary must remain authenticated connection bootstrap only');
assert.match(runner, /protocol hello/, 'base browser protocol-hello bootstrap timeout must remain explicitly classified');
...
assert.doesNotMatch(runner, /RETRYABLE\s*=\s*\/.+(?:SHA|CRC|abort|integrity|payload|transfer|protocol failure)/i,
