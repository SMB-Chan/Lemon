...
assert.deepEqual(A.parseInvite(invite), { peerId: 'drop-testpeer2', secret: zeroSecret, invite });
assert.deepEqual(A.parseInvite('https://example.test/lemon/#peer=' + encodeURIComponent(invite)), {
  peerId: 'drop-testpeer2', secret: zeroSecret, invite,
});
...
assert.deepEqual(P.normalizeFeatures(['flow-control-v1', 'direct-save-v1', 'flow-control-v1', 'BAD FEATURE', '', 3]), [
  'direct-save-v1', 'flow-control-v1',
]);
...
for (const file of ['auth.js', 'capabilities.js', 'diagnostics.js', 'pairing-ui.js', 'auth-guard.js', 'app.js']) {
...
assert.ok(
  ...
  capAt > coreAt && diagAt > capAt && pairingAt > diagAt && guardAt > pairingAt && appAt > guardAt,
...
for (const required of [
  ...
  "style-src 'self'", "connect-src 'self' https://0.peerjs.com wss://0.peerjs.com",
]) {
...
async function testDirectSink() {
  ...
  let aborts = 0;
  const writable = {
    async write(buffer) { writes.push(Buffer.from(new Uint8Array(buffer)).toString('hex')); },
    async close() { closes++; },
    async abort() { aborts++; },
  };
